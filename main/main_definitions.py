...
def system_visual(wave,Ms,Rs,Ts,a,Rp,b,Ag,AB,varep,Porb,T0,Td,Tn):
    ...
    plt.tight_layout()
    plt.savefig('example/system_visual.png',dpi=500)
    plt.show()
    return
...
def LC_plotter(blind,hilite,zoom,t_vz,data,a_mod,d_mod,f_mod,brown,bwn_mod,edep,sig_e,D_e):
    ...
    # plt.scatter(t_vz,data,c='k',s=0.5,alpha=0.5,label='Data')
    if blind == False:
        plt.plot(t_vz,a_mod,c='b',lw=1,label='Light Curve')
        # plt.plot(t_vz,d_mod,c='r',lw=0.5*lwid,alpha=alp, label = 'Detector Model')
        # plt.plot(t_vz,f_mod,c='m',lw=0.5*lwid,alpha=alp)
        plt.axhline(1.0,c='#fec44f',ls='--',lw=1.5,label = 'Stellar Flux')
        if brown == True:
    ...
    plt.xlim(0,t_vz[-1])
    plt.ylim(np.amin(data),np.amax(data)+0.001)
    plt.gca().get_yaxis().get_major_formatter().set_useOffset(False)
    # plt.xlabel('Time (hrs)', fontname='Times New Roman', size=12)
    plt.ylabel('Normalized Flux', fontname='Times New Roman', size=12)
    # plt.title(r'Light Curve - $\mathbb{S}_{e} = %.2f$ - $\Delta D_{e} \approx %.2f$' %
    #           (sig_e,D_e),size=24);

    plt.tight_layout()
    # plt.legend(loc=4)
    plt.savefig('example/stellar_flux.png',dpi=500)
    plt.show()
    ...
    if blind == False:
        plt.plot(t_vz,a_mod,c='b',lw=1,label='Light Curve')
        plt.plot(t_vz,d_mod,c='r',lw=0.5*lwid,alpha=0.5, label = 'Detector Model')
        # plt.plot(t_vz,f_mod,c='m',lw=0.5*lwid,alpha=alp)
        # plt.axhline(1.0,c=(1,0,0),ls='--',lw=1.5,label = 'Stellar Flux')
        if brown == True:
            plt.plot(t_vz,bwn_mod,c=(0.5,0.25,0),lw=lwid,alpha=alp)
        if zoom == True:
            plt.ylim(1.0 - 2.0*edep,1.0 + 2.0*edep)
    ...
    plt.tight_layout()
    # plt.legend(loc=4)
    plt.savefig('example/stellar_detector.png',dpi=500)
    plt.show()

    plt.figure(figsize=(5,3))
    plt.scatter(t_vz,data,c='k',s=0.5,alpha=0.7,label='Data')
    if blind == False:
        # plt.plot(t_vz,a_mod,c='b',lw=1,label='Light Curve')
        # plt.plot(t_vz,d_mod,c='r',lw=0.5*lwid,alpha=alp, label = 'Detector Model')
        # plt.plot(t_vz,f_mod,c='m',lw=0.5*lwid,alpha=alp)
    ...
    plt.tight_layout()
    # plt.legend(loc=4)
    plt.savefig('example/stellar_data.png',dpi=500)
    plt.show()

    return

def jackson_plot(vx_A,vy_A,contrs,t_vz,xo,yo,lxo,hxo,lyo,hyo,Vm,mp_bmask,bounds,mnv,mxv):
    ...
    plt.xlim([0,t_vz[-1]]);
    plt.xlabel('Time (hr)',size=12);
    plt.ylabel('$y_{c}$',size=12);
    ...
    # plt.xlim([np.amin(xo),np.amax(xo)])
    plt.ylim([np.amin(yo),np.amax(yo)])
    plt.gca().set_aspect('equal', adjustable='box')

    # plt.tight_layout()
    plt.savefig('example/smap_centroid.png',dpi=500)
    plt.show()
...
def full_pixelsens(vx_A,vy_A,Vm,contrs,xo,yo):
    ...
    # plt.tight_layout()
    plt.savefig('example/smap.png',dpi=500)
    plt.show()
...
def cent_sensfigs(t_vz,xo,yo,lxo,hxo,lyo,hyo,Vm,mp_bmask,bounds,mnv,mxv):
    ...
    plt.savefig('example/cent_sensfigs.png',dpi=700)
    plt.show()
...
def RBF_Cov_Matrix(x,variance,lengthscale):
	L = lengthscale
	n = len(x)
	d = np.tile(x,(n,1))
	d = d - d.T
	D = (0.5 * 1/(L*L))*np.multiply(d,d.T)
	K = math.pow(variance,2)*np.exp(D)
	return K

def Matern32_Cov_Matrix(x,variance,lengthscale):
	L = lengthscale
	n = len(x)
	d = np.tile(x,(n,1))
	d = (math.sqrt(3)/L)*np.abs(d - d.T)
	K = math.pow(variance,2)*(1.0 + d)*np.exp(-1.0*d)
	return K

def compute_Cov_Matrix(features,hyperparameters):
	# unwrapping parameters
	t, cx, cy, y = features
	Axy, At, Lx, Ly, Lt, error_on_y = hyperparameters
	Kxy = np.exp((-0.5/(math.pow(Lx,2)))*np.square(cx[:,np.newaxis] - cx) + (-0.5/(math.pow(Ly,2)))*np.square(cy[:,np.newaxis] - cy))
	# computing time Matern 3/2 covariance matrix
	Kt = (math.sqrt(3)/abs(Lt)) * np.abs(t[:,np.newaxis] - t)
	Kt = (1.0 + Kt) * np.exp(-1.0*Kt)
	# return total covariance matrix
	return math.pow(Axy,2) * Kxy + math.pow(At,2) * Kt

def alt1_compute_Cov_Matrix(features,hyperparameters):
	t, cx, cy, y = features
	Axy, At, Lx, Ly, Lt = hyperparameters
	Kx = RBF_Cov_Matrix(cx,Axy,Lx)
	Ky = RBF_Cov_Matrix(cy,1.0,Ly)
	Kt = Matern32_Cov_Matrix(t,At,Lt)
	K = np.multiply(Kx,Ky) + Kt
	return K

def alt2_compute_Cov_Matrix(features,hyperparameters): # no unwrapping features, maybe useful for larger data sets
	# unwrapping parameters
	# t, cx, cy, y = features
	Axy, At, Lx, Ly, Lt = hyperparameters
	# computing xy centroid RBF cov matrix
	Kxy = np.exp((-0.5/(math.pow(Lx,2)))*np.square(features[1][:,np.newaxis] - features[1]) + (-0.5/(math.pow(Ly,2)))*np.square(features[2][:,np.newaxis] - features[2]))
	# computing time Matern 3/2 covariance matrix
	Kt = (math.sqrt(3)/abs(Lt)) * np.abs(features[0][:,np.newaxis] - features[0])
	Kt = (1.0 + Kt) * np.exp(-1.0*Kt)
	# return total covariance matrix
	return math.pow(Axy,2) * Kxy + math.pow(At,2) * Kt

# def log_likelihood_all(theta_all,features,t_full,t_dummy):
#     '''
#     Function determining log_likelihood with all parameters (hypers and regular) variable. Variable assignment convention is as follows:
#
#     features = (t, cx, cy, y)
#     hypers = (Axy, At, Lx, Ly, Lt, error_on_y)
#     astro = (amp, Porb, phi_off, dlt_flux, tr_cent)
#     ecl = (ehwid, ineg, edepth)
#     trans = (thwid, tpow, tdepth)
#     theta_all = (astro,ecl,trans,hypers)
#     '''
#
#     #unwrap parameters
#     t, cx, cy, y = features
#     astro, ecl, trans, hypers = theta_all[0:5], theta_all[5:8], theta_all[8:11], theta_all[11:17]
#
#     amp,Porb,phi_off,dlt_flx,tr_cent = astro
#     ehwid,ineg,edepth = ecl
#     thwid,tpow, tdepth = trans
#     Axy, At, Lx, Ly, Lt, error_on_y = hypers
#
#     n = len(t)
#
#     astro_mdl = perf_astro_model(t_full,t_dummy,astro,trans,ecl)
#     res = y - astro_mdl
#
#     Sig = error_on_y * np.identity(n)
#     K = compute_Cov_Matrix(features,hypers)
#     KSig = K+Sig
#
#     inv_KSig = linalg.cholesky(KSig)
#     logdet_KSig = linalg.slogdet(KSig)[1]
#     goodness = (np.dot(np.dot(res,inv_KSig),np.transpose(res)))
#     penalty = logdet_KSig
#     logP = goodness + penalty
#     print theta_all
#     print 'logP: %f' % logP
#     print 'goodness of fit: %f' % goodness
#     print 'penalty: %f' % penalty
#     return logP

def log_likelihood_hypers(theta_hyp,features,params,n_cadence):
    '''
    Function determining log_likelihood with only hyperparameters variable. Variable assignment convention is as follows:
    ...
    trans = (thwid, tpow, tdepth)
    theta_hyp = (hypers)
    params = (astro,ecl,trans)
    '''

    #unwrap parameters
    t, cx, cy, y = features
    astro, ecl, trans = params[0:5], params[5:8], params[8:11]
    hypers = theta_hyp
    ...
    goodness = (np.dot(res, cho_solve((factor,flag),res)))
    logdet = 2*np.sum(np.log(np.diag(factor)))
    length = n * np.log(2*np.pi)
    logP = -0.5 * (goodness + logdet + length)

    print 'Axy: %f, At: %f, Lx: %f, Ly: %f, Lt: %f, Noise: %f' % (theta_hyp[0], theta_hyp[1], theta_hyp[2], theta_hyp[3], theta_hyp[4], theta_hyp[5])
    print 'logP: %f' % logP
    print 'Goodness of Fit: %f' % goodness
    # print 'Penalty: %f' % penalty
    return logP

def log_likelihood_params(theta_par,features,factor,flag,n_cadence):
    '''
    Function determining log_likelihood with only regular parameters variable. Variable assignment convention is as follows:
    ...
    print 'Astro - Amp: %f, Period: %f, Phase: %f, Dlt_flux: %f, Transit Center: %f' % (astro[0],astro[1],astro[2],astro[3],astro[4])
    print 'Eclipse - Halfwidth: %f, Ingress duration: %f, Depth: %f' % (ecl[0],ecl[1],ecl[2])
    print 'Transit - Halfwidth: %f, Power: %f, Depth: %f' % (trans[0],trans[1],trans[2])
    print 'logP: %f' % logP
    print 'Goodness of Fit: %f' % goodness
    #print 'Penalty: %f' % penalty
    return logP

def log_prior_hypers(theta_hyp):
    #unwrap parameters
    hypers = theta_hyp
    Axy, At, Lx, Ly, Lt, error_on_y = hypers

    if (0.0 < Axy < 2.0 and 0.0 < At < 2.0 and 0.0001 < Lx < 1.0 and 0.0001 < Ly < 1.0 and 100.0 < Lt < 10000.0 and 0.00005 < error_on_y < 0.006):
        return 0.0
    return -np.inf

def log_prior_params(theta_par,features,factor,flag,n_cadence):
    #unwrap parameters
    astro, ecl, trans = theta_par[0:5], theta_par[5:8], theta_par[8:11]
    amp,Porb,phi_off,dlt_flx,tr_cent = astro
    ehwid,ineg,edepth = ecl
    ...
    if (1.0e-03 < amp < 5.0e-02 and 3.4e+05 < Porb < 3.6e+05 and 0.0 < phi_off < 2*np.pi and 1.0e-02 < dlt_fux < 1.0e-01 and 2.6e+05 < tr_cent < 2.7e+05 and 1.0e+03 < ehwid < 1.0e+04 and 5.0e+03 < ineg < 6.0e+03 and 0.0 < edepth < 0.1 and 1.0e+03 < thwid < 2.0e+04 and 3.0 < tpow < 5.0 and 0.0 < tdepth < 1.0):
        return 0.0

    return -np.inf

def log_prob_hypers(theta_hyp,features,params,n_cadence):
    lp = log_prior_hypers(theta_hyp)

    if np.isfinite(lp):
        ll = log_likelihood_hypers(theta_hyp,features,params,n_cadence)
        return lp + ll
    else:
        return -np.inf

def log_prob_params(theta_par,features,factor,flag,n_cadence):
    lp = log_prior_params(theta_par)
    if np.isfinite(lp):
        ll = log_likelihood_params(theta_par,features,features,flag,n_cadence)
        return lp + ll
    else:
        return -np.inf
