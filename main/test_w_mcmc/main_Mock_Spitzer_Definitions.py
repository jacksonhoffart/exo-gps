...
def LC_plotter(blind,hilite,zoom,t_vz,data,a_mod,d_mod,f_mod,brown,bwn_mod,edep,sig_e,D_e):
    ...
    plt.figure(figsize=(12,6))
    plt.scatter(t_vz,data,c='k',s=1,alpha=0.75,label='Data')
    if blind == False:
        plt.plot(t_vz,a_mod,c='b',lw=lwid,label='Astro Model')
        plt.plot(t_vz,d_mod,c='r',lw=0.5*lwid,alpha=alp, label = 'Detector Model')
        plt.plot(t_vz,f_mod,c='m',lw=0.5*lwid,alpha=alp)
        plt.axhline(1.0,c=(0.75,0.75,0),ls='--',lw=2,label = 'Stellar Flux')
        if brown == True:
    ...
    plt.tight_layout()
    plt.legend()
    plt.savefig('example/LC_fitter.png',dpi=700)
    plt.show()
...
def full_pixelsens(vx_A,vy_A,Vm,contrs,xo,yo):
    ...
    plt.tight_layout()
    plt.savefig('example/full_pixelsens.png',dpi=700)
    plt.show()
